...
namespace Test.BookingApi
{
    public static class Log
    {
        ...
        {
            var file = $"Test.BookingApi.{fileName}";
            using var s = typeof(Log).Assembly.GetManifestResourceStream(file);
        ...
        public static (ReservationsController, ReservationDto) LoadReservationsControllerPostScenario(string json)
        {
            ...
            ReservationDto dto = logEntry.entry.input.dto.ToObject<ReservationDto>();

            var interactions = logEntry.interactions.ToObject<IEnumerable<Interaction>>();
            ...
            var tables = new[] { new Table(2), new Table(4), new Table(6), new Table(8) };
